...
def get_target(phn_location,dict_timit, nframes, input_size):
    ...
    phn_apposition = phn_file.readlines()
    phn_file.close()

    phn_apposition_length = phn_apposition.__len__() -1
    ...
    #get first phonem
    phn_count = 0
    low_bound, hight_bound, symbol = phn_apposition[phn_count].split(" ")
    ...
    for i in xrange(input_size):
        ...
        if hight_bound > threshold:
            ...
            #get next phonem
            phn_count = min(phn_count+1, phn_apposition_length)
            low_bound, hight_bound, symbol = phn_apposition[phn_count].split(" ")
            low_bound = int(low_bound)
            hight_bound = int(hight_bound)
            symbol = symbol.rstrip()
