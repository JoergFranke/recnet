...
class LSTMlayer(layerMaster):
    ...
    def __init__(self, rng, trng, n_in, n_out, n_batches, old_weights=None,go_backwards=False): #, prm_structure, layer_no ):
        ...
        if old_weights == None:
            ...
            np_weights['w_fg_c'] = self.init_peephole_weight(self.rng, n_out)+2 #Forgot gate with +2 initialized for keeping sequences right from begin
    ...
    def t_forward_step(self,mask, cur_w_in_sig,pre_out_sig,pre_cell_sig, w_ig_c,w_fg_c,w_og_c,w_ifco,b_ifco,t_n_out):
        ...
        #Forget Gate
        fg_t1 = gate_act(T.add( ifco[:,1*t_n_out:2*t_n_out], T.mul(pre_cell_sig , w_fg_c) , cur_w_in_sig[:,1*t_n_out:2*t_n_out]))
        #Cell State
        cs_t1 = T.add( T.mul(fg_t1 , pre_cell_sig), T.mul(ig_t1 , inner_act(T.add(ifco[:,2*t_n_out:3*t_n_out], cur_w_in_sig[:,2*t_n_out:3*t_n_out]))))
        ...
        #Output Gate
        og_t1 = gate_act(T.add( ifco[:,3*t_n_out:4*t_n_out] , T.mul(cs_t1 , w_og_c) , cur_w_in_sig[:,3*t_n_out:4*t_n_out] ))
        #Output LSTM
